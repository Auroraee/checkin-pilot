...
> 更新（2026-08-16）：凭据边界从"页面进程"细化为"浏览器进程内存且不落盘"——`same-origin-refresh` 默认在扩展 service worker 内静默完成 refresh 与签到（host 权限携带 Cookie，Bearer 仅存于 transport 局部变量，不进入扩展消息、存储、日志或通知）；仅当静默 refresh 被判定未认证时，回退到同源页面 ISOLATED world 流程，以兼容校验请求 Origin 的部署。
